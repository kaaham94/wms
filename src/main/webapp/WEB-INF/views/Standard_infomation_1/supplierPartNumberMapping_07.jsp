						...
						<div class="search_box">
							...
							<form name='search' id='search'>
								...
								<div class="col-sm-3">                                   
									<div class="input-group table-responsive">
										<span class="input-group-btn">
											<span class="form-control" style="width:100px;">거래처품번</span>
										</span>
										<input type="text" id="CMATNR" name="CMATNR" value="" onblur="removeSpace(this);" class="form-control">
									</div>
								</div>	
								...
								<div class="col-sm-3">                                   
									<div class="input-group table-responsive">
										<span class="input-group-btn">
											<span class="form-control" style="width:100px;">HK 품명</span>
										</span>
...
function frmScanSubmit(){	
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/supplierPartNumberMapping_07/supplierPartNumberMapping_07Select.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
		...
		success : function(data){ 
			...
			//코드종류 Select
			Select_array = data['Select_CUSTCD'];
			
			for(var j = 0 ; j < Select_array.length; j ++ ){
				Select_list += "<option value="+Select_array[j].CUSTCD_CODE+">"+Select_array[j].CUSTCD+"</option>";
			}
			...
			for(var i = 0 ; i < data['list'].length ; i ++ ){
				...
				CUSTCD_CODE = data['list'][i].CUSTCD_CODE;
				CMATNR = data['list'][i].CMATNR;
				
				CUSTCD_CODE = CUSTCD_CODE.replace(/(\s*)/g, "");
				CMATNR = CMATNR.replace(/(\s*)/g, "");
				
						list += "<tr name='defaultTd'>"+
								"	<td class='text-align-center'>"+
								"		<label>"+
								"			<input type='checkbox' name='chk[]' id='"+CUSTCD_CODE + CMATNR+"' value='' onclick='check(this)'><span class='text'></span>"+
								"		</label>"+
								"    </td>"+
								"    <td value='"+ data['list'][i].CUSTCD_CODE +"' class='text-align-left'>"+ data['list'][i].CUSTCD +"</td>"+
								"    <td class='text-align-left'>"+ data['list'][i].CMATNR +"</td>"+
								"    <td class='text-align-left'>"+ data['list'][i].MAKTX +"</td>"+
								...
								"    <td class='text-align-center'>"+ data['list'][i].MKUSER +"</td>"+
								"    <td class='text-align-center'>"+ data['list'][i].MODATE +"</td>"+
								"    <td class='text-align-center'>"+ data['list'][i].MOUSER +"</td>"+
								"</tr>";
			}
