								...
								<div class="col-sm-3">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-btn">
                                            <select class="form-control" id="Term" style="width:100px;">
								            ...
								            <c:when test="${fn:length(list) > 0}">
								                <c:forEach items="${list }" var="row">
								                    <tr>
								                        ...
								                        <td class="text-align-center"><a href="javascript:detail(${row.number });">${row.id }</a></td>
								                        <td class="text-align-center">${row.passwd }</td>
								                        <td class="text-align-center">${row.name }</td>
								                        <td class="text-align-center">${row.position }</td>
								                        <td class="text-align-center">${row.date }</td>
								                        <td class="text-align-center"><c:if test="${row.modified_date != row.date }" >${row.modified_date}</c:if></td>
								                        <td class="text-align-center">${row.recent_visit }</td>
								                        <td class="text-align-center">${row.ipAdd }</td>
								                    </tr>
								                </c:forEach>
								            </c:when>
								            <c:otherwise>
...
<%@ include file ="/WEB-INF/views/include/include_foot.jsp" %>
	...
	<form id="frmDownloadExcel" method="post" action="/sys/excelDown_account.do">
...
$('#plant').change(function() {
	...
	setCookie( 'session', selectedText, 365);	//지정한 행값을 쿠키값으로 저장한다.
...
function fn_excel_all_down() {
	...
	comSubmit.setUrl("<c:url value='/sys/excelDown_account.do' />");
	$("#param").val("total");
	comSubmit.submit();
}
//-현재 
...
function detail(number) { 
	...
	var comSubmit = new ComSubmit("frmSearch");
    comSubmit.setUrl("<c:url value='/sys/view_account.do' />");
...
function write() {
		...
		var comSubmit = new ComSubmit();
	       comSubmit.setUrl("<c:url value='/sys/write_account.do' />");
	       comSubmit.submit();
}	
</script>
