...
/*
var active = false;
$(document).on('mousedown','[name="defaultTd"]',function(ev){
	active = true;
...
$(document).on('mousemove','[name="defaultTd"]',function(ev){
	if (active) {
		$(this).children().addClass("highlight");
	}
});
...
function LGORT(MANDT, WERKS){
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/stockStatus_06Select_LGORT.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
...
function frmScanSubmit(pageNo){	
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/stockStatus_06Select.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
		...
		success : function(data){
			...
			for(var i = 0 ; i < data['list'].length-3 ; i ++ ){
				...
				if(data['list'][i].rowspan3 !== undefined){
					rowspan3 = "<td style='vertical-align:top;' class='text-align-center rowspan3' style='background-color: #f9f9f9; font-size:16px;' rowspan='"+data['list'][i].rowspan3+"'><b class='textFixing'>"+ data['list'][i].LOCNAME +"</b></td>" 
				...
				list = "<tr name='defaultTd'>"+
						...
						"<td class='text-align-center'>"+ data['list'][i].BOXCNT +"</td>"+     	//박스수 						"<td class='text-align-center'>"+ data['list'][i].POSNR +"</td>"+ 	  //출고 요청순번 
