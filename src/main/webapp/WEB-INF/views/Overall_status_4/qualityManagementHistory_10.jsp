								...
								<div class="col-sm-3">   
                                    <div class="input-group table-responsive">
										<span class="input-group-btn">
											<span class="form-control" style="width:100px;">조회기간</span>
										</span>
...
function LGORT(MANDT, WERKS){
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/qualityManagementHistory_10Select_LGORT.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
...
function frmScanSubmit(pageNo){	
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/qualityManagementHistory_10Select.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
		...
		success : function(data){
			...
			for(var i = 0 ; i < data['list'].length-3 ; i ++ ){
				...
				if(data['list'][i].rowspan3 !== undefined){
					rowspan3 = "<td style='vertical-align:top; word-break:break-all;' wrap='hard' class='text-align-left rowspan3' style='background-color: #f9f9f9; font-size:16px;' rowspan='"+data['list'][i].rowspan3+"'><b class='textFixing'>"+ data['list'][i].BARNO +"</b></td>" 
				...
				list = "<tr name='defaultTd'>"+
						...
						rowspan2 +	//저장위치
						rowspan3 +	//바코드
						"<td class='text-align-left'>"+ data['list'][i].MAKTX +"</td>"+   		//품번
						"<td class='text-align-right'>"+ numberWithCommas(data['list'][i].QTY) +"</td>"+  		//수량
						"<td class='text-align-left'>"+ data['list'][i].PRESTATUS +"</td>"+		//이전상태
						"<td class='text-align-left'>"+ data['list'][i].STATUS +"</td>"+ 	 	//현재상태
						"<td class='text-align-left'>"+ data['list'][i].REASONNAME +"</td>"+ 	//사유
						"<td class='text-align-left'>"+ data['list'][i].STAMP +"</td>"+ 	  	//처리일시
						"<td class='text-align-left'>"+ data['list'][i].USERNAME +"</td>"+    	//처리자
						"<td class='text-align-left'>"+ data['list'][i].REMARK +"</td>"+ 	  	//비교 
						"<td class='text-align-left'>"+ data['list'][i].PROCTYPE +"</td>"+		//처리구분
					"</tr>";
			...
			$('#STATUS').append(Select_list_STATUS);  	//상태
			
			//select 선택값 return 
			$("#WERKS").val(data['map'].WERKS ); 		//법인
			$("#STATUS").val(data['map'].STATUS ); 		//상태
...
$(document).ready(function() {
    ...
    if(getParameterByName("MANDT") == ""){
    	...
    	cb( moment(getParameterByName("FDATE"), 'YYYY-MM-DD'), moment(getParameterByName("EDATE"), 'YYYY-MM-DD') );
    	get_query();
    }
