...
function REPRINT(){
	...
	if(confirm(  "현재 선택된 바코드들에 대해서 선택된 프린트로 재발행 등록 처리하시겠습니까?" )){  //1.1 진행여부 : Yes
		...
		if(save_chk == true){
			//ajax 시작 
			 $.ajax({
		 			url:"/WMS/Overall_status_4/dailyDetailStockStatus_09InsertUpdate_REPRINT.do",
		 			type:"post",
...
function LGORT(MANDT, WERKS){
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/dailyDetailStockStatus_09Select_LGORT.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
...
function frmScanSubmit(pageNo){	
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/dailyDetailStockStatus_09Select.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
		...
		success : function(data){
			...
			for(var i = 0 ; i < data['list'].length-3 ; i ++ ){
				...
				if(data['list'][i].rowspan2 !== undefined){
					rowspan2 = "<td style='vertical-align:top;' class='text-align-center rowspan2' style='background-color: #f9f9f9; font-size:16px;' rowspan='"+data['list'][i].rowspan2+"'><b class='textFixing'>"+ data['list'][i].WERKS +"</b></td>" 
				...
				if(data['list'][i].rowspan3 !== undefined){
					rowspan3 = "<td style='vertical-align:top;' class='text-align-center rowspan3' style='background-color: #f9f9f9; font-size:16px;' rowspan='"+data['list'][i].rowspan3+"'><b class='textFixing'>"+ data['list'][i].LGORT +"</b></td>" 
