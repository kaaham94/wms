...
function frmScanSubmit(pageNo){	
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/materialsDetail_03Select.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
		...
		success : function(data){
			...
			for(var i = 0 ; i < data['list'].length-3 ; i ++ ){
				...
				list = "<tr name='defaultTd'>"+
							...
							"<td style='word-break:break-all;' wrap='hard' class='text-align-center'>"+ data['list'][i].BARNO +"</td>"+ //BARNO
							"<td class='text-align-center'>"+ data['list'][i].BCQTY +"</td>"+ //수량
							"<td class='text-align-center'>"+ data['list'][i].LIFNRNM +"</td>"+ //공급처
							"<td class='text-align-center'>"+ data['list'][i].KUNNRNM +"</td>"+ //출하저
							...
							"<td class='text-align-center'>"+ data['list'][i].CRDTD +"</td>"+ //입고일시
							"<td class='text-align-center'>"+ data['list'][i].USERNAME +"</td>"+ //입고자
						"</tr>";
