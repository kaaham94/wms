						...
						<div class="search_box">
							...
							<form name='search' id='search'>
								...
								<div class="col-sm-3">   
                                    <div class="input-group table-responsive">
										<span class="input-group-btn">
											<span class="form-control" style="width:100px;">출고일</span>
										</span>
								...
								<div class="col-sm-3">   
									...
									<div class="input-group table-responsive">
										<span class="input-group-btn">
											<span class="form-control" style="width:100px;">출고요청번호</span>
										</span>
										<input type="text" id="VBELN" name="VBELN" value="" onblur="removeSpace(this);" class="form-control">
									</div>
									<div class="input-group table-responsive">
										<span class="input-group-btn">
											<span class="form-control" style="width:100px;">바코드NO</span>
										</span>
								...
								<div class="col-sm-3">   
									...
									<div class="input-group table-responsive">
										<span class="input-group-btn">
											<span class="form-control" style="width:100px;">고객</span>
										</span>
...
function LGORT(MANDT, WERKS){
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/detailStatusOfShipment_05Select_LGORT.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
...
$(document).on('change','[id="WERKS"]',function(e){
...
function frmScanSubmit(pageNo){	
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/Overall_status_4/detailStatusOfShipment_05Select.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
		...
		success : function(data){
			...
			var Select_array_CUST =  new Array(); 		//고객 
			...
			for(var i = 0 ; i < data['list'].length-3 ; i ++ ){
				...
				if(data['list'][i].rowspan !== undefined){
					rowspan = "<td style='vertical-align:top;' class='text-align-center rowspan1' style=' background-color: #f9f9f9; font-size:17px;' rowspan='"+data['list'][i].rowspan+"'><b class='textFixing'>"+ data['list'][i].WERKSNM +"</b></td>" 
				}else{
					rowspan = "";
				}
				...
				if(data['list'][i].rowspan2 !== undefined){
					rowspan2 = "<td style='vertical-align:top;' class='text-align-center rowspan2' style='background-color: #f9f9f9; font-size:16px;' rowspan='"+data['list'][i].rowspan2+"'><b class='textFixing'>"+ data['list'][i].LGORTNM +"</b></td>" 
				...
				if(data['list'][i].rowspan3 !== undefined){
					rowspan3 = "<td style='vertical-align:top;' class='text-align-center rowspan3' style='background-color: #f9f9f9; font-size:16px;' rowspan='"+data['list'][i].rowspan3+"'><b class='textFixing'>"+ data['list'][i].SUDAT +"</b></td>" 
				...
				list = "<tr name='defaultTd'>"+
						...
						rowspan2 +	//저장위치
						rowspan3 +	//출고일
						"<td class='text-align-center'>"+ data['list'][i].BWARTNM +"</td>"+   //출고유형
						"<td class='text-align-center'>"+ data['list'][i].KUNNRNM +"</td>"+   //납품처
						"<td class='text-align-center'>"+ data['list'][i].CUSTNM +"</td>"+ 	  //제조사
						"<td class='text-align-center'>"+ data['list'][i].MAKTX +"</td>"+ 	  //품번
						"<td class='text-align-center'>"+ data['list'][i].BARNO +"</td>"+ 	  //BARNO
						"<td class='text-align-center'>"+ data['list'][i].BCQTY +"</td>"+ 	  //수량
						"<td class='text-align-center'>"+ data['list'][i].VBELN +"</td>"+     //출고 요청번호
						"<td class='text-align-center'>"+ data['list'][i].POSNR +"</td>"+ 	  //출고 요청순번 
						"<td class='text-align-center'>"+ data['list'][i].CRDTD +"</td>"+	  //출고일시
						"<td class='text-align-center'>"+ data['list'][i].USERNAME +"</td>"+	  //출고자
					"</tr>";
