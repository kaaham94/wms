...
function frmScanSubmit(){
	...
	// ajax({ 시작
	$.ajax({
		url : '/WMS/groupAuthority_03/groupAuthority_03Select_left.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
		...
		success : function(data){ 
			...
			for(var i = 0 ; i < data['list'].length ; i ++ ){
				...
				list += "<tr name='defaultTd'>"+
						"    <td class='text-align-center' name='ROLLID'>"+ data['list'][i].CODE +"</td>"+ 	//그룹ID
						"    <td class='text-align-left'>"+ data['list'][i].CODENAME +"</td>"+ 	//그룹명	
						"    <td class='text-align-left'>"+ data['list'][i].ETC +"</td>"+		//내역
						"</tr>";
			}
...
function right_table(MANDT, language, ROLLID, groupName){	
	//Create an FormData object 
	var params =  "MANDT=" + MANDT + "&language=" + language + "&ROLLID=" + ROLLID;
	
	// ajax({ 시작
	$.ajax({
		url : '/WMS/groupAuthority_03/groupAuthority_03Select_right.do',
		type: "post",
		dataType : 'json',
		data: params,
		async: false,
...
$(document).on('click','#left_table [name="defaultTd"]',function(e){
	...
	var ROLLID =  $(this).children().eq(0).text();
	var groupName =  $(this).children().eq(1).text();
	var MANDT = $('#MANDT').val();
	var language ="KOREAN";
...
$("#res").on("click", function() {
	...
	if(confirm(  "저장 하시겠습니까?" )){  //1.1 진행여부 : Yes
		...
		var ROLLID = $("td[name='ROLLID']"); //
		
		var ROLLID_value="";
		for(var i = 0; i<ROLLID.size(); i++){
			if(ROLLID.eq(i).attr('style') == "background-color: gold;"){
				ROLLID_value = ROLLID.eq(i).text();
			}
		}
		...
		for(var i = 0; i<PROGID.size(); i++){
			...
			insertArray.push({
				MANDT     : "100", 				  //클라이언트
				ROLLID    : ROLLID_value,  		  //권한그룹ID
				PROGID    : PROGID.eq(i).text(),  //프로그램ID (=화면ID)
				FINDGRANT : FINDGRANT, 			  //조회권한:I(등록),U(업데이트),D(삭제) 
				NEWGRANT  : NEWGRANT,  			  //신규권한
				SAVEGRANT : SAVEGRANT,  		  //저장권한
				DELGRANT  : DELGRANT,  			  //삭제권한
				EXPGRANT  :  ""  	,  			  //엑셀권한
				PRTGRANT  :  ""  	,  			  //출력권한
				UPDUSER   : user   				  //수정자(=USERID)
			});
		...
		if(save_chk == true){
			//ajax 시작 
			 $.ajax({
		 			url:"/WMS/Management_of_workers_3/groupAuthority_03InsertUpdate.do",
		 			type:"post",
